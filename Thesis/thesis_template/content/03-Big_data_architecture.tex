...
As outlined in Section [add section number], one could use a relational database, such as MySQL or PostgreSQL.
In this case, one would create a Representational state transfer (REST) API for communication between client and the database.
As a consequence of chosen RESTful communication, transfer of stateless messages via HTTP is used.
